...
/**
 * Solution implements of Dinic's network flow algorithm.
 *
 * The algorithm works by first constructing
 * a level graph using a BFS and then finding
 * augmenting paths on the level graph using
 * multiple DFSs.
 *
...
public class Solution {
  ...
  private abstract static class NetworkFlowSolverBase {
    ...
    private void initializeEmptyFlowGraph() {
      graph = new List[n];
      for (int i = 0; i < this.n; i++) graph[i] = new ArrayList<Edge>();
    }
    ...
    private long dfs(int at, int[] next, long flow) {
      ...
      for (; next[at] < numEdges; next[at]++) {
        ...
        if (cap > 0 && level[edge.to] == level[at] + 1) {
          long min = flow > cap ? cap : flow;
          long bottleNeck = dfs(edge.to, next, min);
          if (bottleNeck > 0) {
  ...
  public static int solution(int[] entrances, int[] exits, int[][] path) {
    ...
    for (int j : exits) {
      solver.addEdge(j, e, INF);
    }

    // Add edges with values > 0
    for ( int i = 0; i <= path.length-exits.length; i++ ) {
      for (int j = 0; j < path.length; j++) {

        // Remove all exit rows as all exit
        // rows contains 0 values
        if (path[i][j] > 0)
          solver.addEdge(i, j, path[i][j]);
      }
    }

    return (int)solver.getMaxFlow();
  }
